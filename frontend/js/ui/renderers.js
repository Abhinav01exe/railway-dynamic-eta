...
(function() {
  ...
  function renderTimeline(stationResults, currentStationIndex, selectedStationIndex, onRelocateTrain, onInspectStation) {
    ...
    // Show only the current station, next 5 stations, and final destination.
    // The complete RailRadar route remains available in the backend.
    const visibleEntries = [];

    for (
      let originalIdx = currentStationIndex;
      originalIdx < Math.min(
      ...
      visibleEntries.push({
        stn: stationResults[originalIdx],
        originalIdx: originalIdx
      });
    }

    const destinationIdx = stationResults.length - 1;

    if (
      destinationIdx >= 0 &&
      !visibleEntries.some(entry => entry.originalIdx === destinationIdx)
    ) {
      visibleEntries.push({
        stn: stationResults[destinationIdx],
        originalIdx: destinationIdx
      });
    }
    ...
    visibleEntries.forEach((entry, displayIdx) => {
      const stn = entry.stn;
      const idx = entry.originalIdx;
      ...
      item.innerHTML = `
        <div class="timeline-track-col">
          ...
          ${markerHtml}
          <div class="track-line bottom-line ${displayIdx === visibleEntries.length - 1 ? "hidden" : ""}"></div>
        </div>
